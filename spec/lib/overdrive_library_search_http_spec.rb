...
describe 'Overdrive::LibrarySearch over HTTP' do
  it 'returns id, name and logo for each nearby consortium' do
    ...
    stub_request(:get, HttpFixtures::FIND_LIBRARIES_URL).to_return(status: 200, body: body)
  ...
  it 'sends the zip code as given, leaving normalization to the caller' do
    ...
    stub_request(:get, HttpFixtures::FIND_LIBRARIES_URL).to_return(status: 200, body: '[]')
  ...
  it 'sends browser-ish headers, without which OverDrive blocks the request' do
    ...
    assert_requested(:get, HttpFixtures::FIND_LIBRARIES_URL, headers: {'Referer' => 'https://www.overdrive.com/libraries'}, times: 1)
  end

  describe 'when OverDrive does not answer with JSON' do
    it 'raises ApiError naming the status on a 403' do
      stub_request(:get, HttpFixtures::FIND_LIBRARIES_URL).to_return(status: 403, body: '<html><body>Access Denied</body></html>')
    ...
    it 'raises ApiError rather than JSON::ParserError when a 200 carries HTML' do
      ...
      stub_request(:get, HttpFixtures::FIND_LIBRARIES_URL).to_return(status: 200, body: '<!DOCTYPE html><html><head><title>Just a moment</title></head></html>')
