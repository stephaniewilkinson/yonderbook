...
describe 'RouteHelpers over HTTP' do
  ...
  let(:request) { FakeRodaRequest.new }
  ...
  describe '#fetch_local_libraries' do
    ...
    it 'falls back to the shelf list when no fallback path is given' do
      stub_request(:get, HttpFixtures::FIND_LIBRARIES_URL).to_return(status: 200, body: '<html>Just a moment</html>')
  ...
  describe '#fetch_and_cache_request_token' do
    it 'returns the token and caches it' do
      stub_request(:post, HttpFixtures::REQUEST_TOKEN_URL).to_return(status: 200, body: 'oauth_token=tok-abc&oauth_token_secret=sec-xyz')
    ...
    it 'returns the cached token without asking Goodreads twice' do
      ...
      assert_requested(:post, HttpFixtures::REQUEST_TOKEN_URL, times: 1)
    end
    ...
    it 'returns nil rather than raising when Goodreads answers 500' do
      ...
      stub_request(:post, HttpFixtures::REQUEST_TOKEN_URL).to_return(status: 500, body: 'upstream error')
